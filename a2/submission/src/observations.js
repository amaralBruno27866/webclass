...
function speciesCoordinates(data) {
  if (data && Array.isArray(data.results)) {
    for (let i = 0; i < data.results.length; i++) {
      if (
        data.results[i] &&
        typeof data.results[i].species_guess === 'string' &&
        typeof data.results[i].location === 'string'
      ) {
        console.log(
  ...
  if (data && Array.isArray(data.results)) {
    data.results.forEach(function (observation) {
      if (
        ...
        console.log(
          `"${observation.species_guess}" observed at coordinates (${observation.location})`
        );
      }
    });
  }
  ...
  if (data && Array.isArray(data.results)) {
    for (let observation of data.results) {
      if (
        ...
        let locationWithSpace = observation.location.replace(',', ', ');
        console.log(
          `"${observation.species_guess}" observed at coordinates (${locationWithSpace})`
        );
      }
    }
  }
...
function observationsByQualityGrade(data, qualityGrade) {
  ...
  if (typeof qualityGrade !== 'string' && qualityGrade !== null) {
    throw new Error('qualityGrade deve ser uma string ou null');
  }

  const filteredObservations = data.results.filter((observation) => {
    return observation.quality_grade === qualityGrade;
  });
...
function observationsByQualityGrades(data, ...qualityGrades) {
  ...
  const filteredObservations = qualityGrades.flatMap((qualityGrade) => {
    return observationsByQualityGrade(data, qualityGrade);
  });
...
function transformObservation(original) {
  ...
  transformed.name = original.species_guess.toLowerCase();

  if (original.conservation_status) {
    transformed.isExtinct =
      original.conservation_status.status_name.toLowerCase() === 'extinct in the wild';
  } else {
    transformed.isExtinct = false;
  }

  transformed.images = original.photos.map((photo) => {
    return {
      url: photo.url,
  ...
  transformed.observer = original.user.login_exact + '@inaturalist.com';
...
function getObservationsByTaxa(data, ...taxaNames) {
  ...
  taxaNames.forEach((taxon) => {
    var observationsForTaxon = data.results.filter((observation) => {
...
function getObservationsByLocation(data, options = {}) {
  return data.results.filter((observation) => {
    var lat = parseFloat(observation.location.split(',')[0]);
    var lng = parseFloat(observation.location.split(',')[1]);

    if (options.lat && options.lng) {
      if (typeof options.lat === 'object' && typeof options.lng === 'object') {
        return (
          lat >= options.lat.min &&
          lat <= options.lat.max &&
          lng >= options.lng.min &&
    ...
    if (options.lat) {
      if (typeof options.lat === 'object') {
        return lat >= options.lat.min && lat <= options.lat.max;
      }
      return lat === options.lat;
    }

    if (options.lng) {
      if (typeof options.lng === 'object') {
        return lng >= options.lng.min && lng <= options.lng.max;
      }
      return lng === options.lng;
    }
...
function getPlaceURLs(data) {
  ...
  data.results.forEach((observation) => {
    observation.place_ids.forEach((place_id) => {
      urls.push('https://www.inaturalist.org/observations?place_id=' + place_id);
    });
  });
...
function getSpeciesObservations(data) {
  ...
  data.results.forEach((observation) => {
    var speciesName = observation.taxon.name;

    if (observationCounts[speciesName]) {
      observationCounts[speciesName]++;
    } else {
      observationCounts[speciesName] = 1;
    }
  });
...
function extractSpeciesNames(data) {
  // Crie um novo array para armazenar os nomes das espécies
  var speciesNames = [];

  // Use um loop forEach para iterar sobre todas as observações no array de dados
  data.results.forEach((observation) => {
    // Se o nome da espécie ainda não estiver no array, adicione-o
    if (!speciesNames.includes(observation.species_guess)) {
      speciesNames.push(observation.species_guess);
    }
  });

  // Retorne o array de nomes de espécies
  return speciesNames;
}
...
function extractSpeciesNames2(data) {
  // Crie um novo conjunto para armazenar os nomes das espécies
  var speciesNames = new Set();

  // Use um loop forEach para iterar sobre todas as observações no array de dados
  data.results.forEach((observation) => {
    // Adicione o nome da espécie ao conjunto
    speciesNames.add(observation.species_guess);
  });

  // Converta o conjunto em um array e retorne-o
