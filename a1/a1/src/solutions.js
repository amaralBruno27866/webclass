...
function parseDateString(value) {
  if (typeof value !== 'string' || !/^(\d{4}\/\d{2}\/\d{2}|\d{2}\/\d{2}\/\d{4})$/.test(value)) {
    throw new Error(
...
function toDateString(value, format) {
  ...
  throw new Error('Unknown date format. Use "YYYY/MM/DD", "DD/MM/YYYY", or "MM/DD/YYYY".');
}
...
function normalizeTime(value) {
  const timeRegex = /^([0-1]?[0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9])\s?(AM|PM)?$/;
  ...
  if (
    hours < 0 ||
    hours > 23 ||
    (period === 'AM' && (hours < 1 || hours > 12)) ||
    (period === 'PM' && (hours < 1 || hours > 12)) ||
    minutes < 0 ||
    minutes > 59 ||
    seconds < 0 ||
    seconds > 59
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  if (!resourceType || !resourceId || extraData === '') {
    throw new Error(
      'Resource type and resource id must be provided, and extraData cannot be an empty string'
    );
  }
